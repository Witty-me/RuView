...
## 附录：宣传稿/评论真实性细化判断（基于当前环境可验证证据）

### 0) 外部材料访问说明

你给的 3 个知乎链接在当前执行环境均返回代理 403（`CONNECT tunnel failed, response 403`），无法直接抓取正文逐段对照。因此本节采用“仓库内公开宣传文案 + 可执行证据”进行真实性细化。
...
- **A档：可证实**（代码/命令能直接验证）
- **B档：部分属实**（有实现或原型，但默认路径不闭环）
- **C档：高风险夸大**（文档承诺显著超过当前可复现实证）

### 2) 代表性宣传点真实性判断

1. **“30秒启动、开箱即用”** → **B档（部分属实）**
   - 属实部分：Docker/二进制入口确实存在。
   - 风险部分：Python 线与验证线在默认环境并不稳；用户容易遇到启动与依赖阻断。

2. **“tests 1100+ / 工程已充分验证”** → **B档（部分属实）**
   - 属实部分：仓库测试体量确实大。
   - 风险部分：并非所有关键链路都能在默认环境顺滑复现（例如 train 路径受 feature 与外部依赖门控）。

3. **“自学习/域泛化/多视角融合已成熟可用”**（ADR-024/027/031/033 风格） → **C档（高风险夸大）**
   - 属实部分：相关模块和代码储备确实存在。
   - 风险部分：主入口默认运行链路未完整消费这些高级模块，缺少统一的端到端可复现实证。

4. **“可验证真实性（proof/verify）”** → **C档（高风险夸大）**
   - 属实部分：确有 verify 机制和数据工件。
   - 风险部分：当前环境执行 `./verify` 为 FAIL（哈希不匹配），不满足“普通用户一键可复现 PASS”的宣传强度。

5. **“Python 与 Rust 是同一能力栈”**（用户常见理解） → **C档（高风险误读）**
   - 现实更接近“双轨并行”：Python v1 是 legacy，Rust 是主线；两者并非单一统一运行时闭环。

### 3) 对外宣传的可信改写建议（可直接用于公关文案）

建议所有宣传语都加“证据锚点”：

- **默认可运行**：给唯一入口命令 + 期望输出 + CI 证明。
- **实验/提案能力**：明确标注 ADR 状态（Proposed/Partially Implemented）。
- **跨环境泛化/自学习**：若无公开可复现实验脚本与指标报告，不要写成“已成熟上线”。

### 4) 当前总评（宣传真实性）

- **工程基础并非空壳**（Rust 多 crate 与算法模块真实存在）。
- **但宣传强度整体高于默认可复现强度**，尤其是“高级智能能力已上线”这类表述。
- 最稳妥的判断：这是一个“实现储备丰富但闭环治理不足”的项目，而不是“完全可量产交付”的状态。

---

## 附录：宣传材料复核补充（基于本地 docs 现状）

### A) 关于你提到的 3 个 `.mhtml` 文件

本轮我先在仓库内做了本地文件检索（`docs/` 与全仓库），当前工作树**未检索到任何 `.mhtml/.mht` 文件**。这意味着我无法像你期望的那样对三份离线网页逐段摘录比对。

> 如果这 3 个文件是通过别的路径挂载（例如未进入当前 git 工作树），下一轮你可以直接给出相对路径，我会按文件逐段对照并更新结论。

### B) 先用仓库内“宣传型文档”做替代复核

在缺少 `.mhtml` 正文的前提下，我改用仓库内以下高宣传强度材料做真实性细化：
...
- `docs/user-guide.md` / `docs/build-guide.md`

### C) 细化结论：哪些宣传是“可证实”、哪些是“叙事超前”

1. **“多静态/多视角几何增强思路正确”** → **A/B 档**
   - 文档与代码层都能找到对应模块（ruvector/viewpoint、signal/mat 相关处理模块），说明方向不是空想。
   - 但“从模块存在到默认主链路稳定启用”中间仍有较大工程鸿沟（入口、feature、依赖与验收样例未统一）。

2. **“SOTA 级精度与端到端可复现”** → **C 档高风险**
   - 研究文档给了大量理论、公式与目标指标，但仓库默认入口缺少一条“新用户可一键复现该指标”的闭环脚本与 CI 证据链。
   - 当前可观测现实更接近“模块储备 + 原型组合”，而非“稳定可重复 benchmark 工程”。

3. **“训练-推理一体化已成熟”** → **C 档高风险**
   - 训练相关 crate 与模块是存在的，但主运行入口（特别在线 sensing 路径）仍可见启发式/fallback 主导场景。
   - 这会导致对外口径若写成“已成熟上线”，与实际默认运行形态不一致。

4. **“真实性验证链（verify）强可复现”** → **C 档高风险**
   - verify 机制和 proof 工件存在（不是空白），但在当前环境仍未稳定 PASS。
   - 这类结论应从“已实现验证机制”降级为“验证机制存在，但复现稳定性待提升”。

### D) 对“宣传真实性”的最新总判（补充版）

- **不是纯 PPT 工程**：有明显真实代码资产与算法实现。
- **也不是宣传等于交付**：越高层 ADR/研究叙事，越容易出现“文档完整 > 运行闭环完整”。
- **建议对外采用分层表达**：
  - 已稳定可复现（附命令与 CI）；
  - 模块可运行但未默认接入；
  - 提案/实验中（给 roadmap，不给上线式承诺）。
